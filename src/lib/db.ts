...
function migrateSchema(database: Database.Database) {
  ...
  const threadCols = database.prepare("PRAGMA table_info(worry_thread_snapshots)").all() as {
    name: string;
  }[];
  if (!threadCols.some((c) => c.name === "bright_threads_json")) {
    database.exec(
      "ALTER TABLE worry_thread_snapshots ADD COLUMN bright_threads_json TEXT NOT NULL DEFAULT '[]'"
    );
  }
  if (!threadCols.some((c) => c.name === "celebrate_recommendation")) {
    database.exec("ALTER TABLE worry_thread_snapshots ADD COLUMN celebrate_recommendation TEXT");
  }
